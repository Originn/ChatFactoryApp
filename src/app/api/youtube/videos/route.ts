...
export async function GET(req: NextRequest) {
  try {
    ...
    const videoDetailsResponse = await fetch(
      `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics,status,localizations&id=${videoIds}&key=${apiKey}`,
      {
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
    ...
    // Fetch transcript information for all videos
    const videosWithTranscripts = await Promise.all(
      videoDetailsData.items.map(async (video: any) => {
        let transcripts = [];
        
        // Check multiple indicators for captions/transcripts
        const hasCaption = video.contentDetails?.caption === 'true';
        const hasDefaultLanguage = video.snippet?.defaultLanguage;
        const hasAudioLanguage = video.snippet?.defaultAudioLanguage;
        
        if (hasCaption || hasDefaultLanguage || hasAudioLanguage) {
          transcripts = [{
            language: hasDefaultLanguage || hasAudioLanguage || 'unknown',
            languageName: hasDefaultLanguage || hasAudioLanguage || 'Available',
            isAutoGenerated: !hasCaption // Assume auto-generated if not explicitly captioned
          }];
        }
    ...
    const filteredVideos = searchQuery 
      ? videosWithTranscripts.filter(video => 
